...
def compute_artifact_relevance(config):
    ...
    for split in [
        ...
        sample_sets_split = {l: [y for y in x if y in split_set] for l, x in zip(artifact_labels, 
                                                                           artifact_sample_ids)}

        for k, (artifact_label, samples) in enumerate(sample_sets_split.items()):
            if dataset_name == "isic" and (artifact_label not in config_name):
                print(f"Skip evaluation of {artifact_label} for {config_name}")
                continue
            dataset = get_dataset(dataset_name)(data_paths=data_paths,
                                                normalize_data=True,
                                                artifact_ids_file=artifacts_file,
            ...
            if len(samples) > 1000:
                num_before = len(samples)
                samples = samples[::10]
                print(f"Reduced n_samples from {num_before} to {len(samples)}")
            ...
            for i in tqdm(range(n_batches)):
                ...
                ## Compute relevance with SHAP
                # Assignments
                assignments = []
                for idx in range(len(data)):
                    img = dataset.reverse_normalization(data[idx]).detach().cpu().numpy().transpose(1,2,0).astype("uint8")
                    assignment = torch.tensor(slic.iterate(img.copy(order='C'))).to(device)
                    assignments.append(assignment)
                assignments_batch = torch.stack(assignments)
                # Run SHAP
                attr = svs.attribute(data, target=targets.long(), 
                                     n_samples=150, feature_mask=assignments_batch.unsqueeze(1), 
                                     show_progress=True, baselines=0)

                attr = attr.sum(1)
                amax = attr.flatten(1).abs().max(1).values[...,None,None]
                attr_norm = attr/amax
                ...
                inside = (attr_norm * mask).abs().sum((1, 2)) / (
                        attr_norm.abs().sum((1, 2)) + 1e-10)
